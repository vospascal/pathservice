...
export function setPath(obj, path, value, doNotReplace) {
...
export function delPath(obj, path) {
  ...
  if (path.length === 1) {
    ...
    return delPath(obj[currentPath], path.slice(1));
  }
...
export function hasPath(obj, path) {
  ...
  for (var i = 0; i < path.length; i++) {
    ...
    if (
      (typeof j === "number" && Array.isArray(obj) && j < obj.length) ||
      (options.includeInheritedProps
        ? j in Object(obj)
